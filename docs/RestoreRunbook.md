...
Run on the EC2 box. Container: `betaname-prod-db-1`.
...
2. Create scratch DB:
   docker exec betaname-prod-db-1 psql -U postgres -c "DROP DATABASE IF EXISTS restore_drill;"
   docker exec betaname-prod-db-1 psql -U postgres -c "CREATE DATABASE restore_drill;"

3. Restore into it:
   docker cp /tmp/restore.dump betaname-prod-db-1:/tmp/restore.dump
   docker exec betaname-prod-db-1 pg_restore -U postgres -d restore_drill --no-owner --no-acl /tmp/restore.dump

4. Verify (don't trust "no error" — check rows):
   docker exec betaname-prod-db-1 psql -U postgres -d restore_drill -c "\dt"
   docker exec betaname-prod-db-1 psql -U postgres -d restore_drill -c "SELECT count(\*) FROM media_item;"

5. Teardown:
   docker exec betaname-prod-db-1 psql -U postgres -c "DROP DATABASE restore_drill;"
   docker exec betaname-prod-db-1 rm -f /tmp/restore.dump
