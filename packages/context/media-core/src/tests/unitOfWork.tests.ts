 ...
 * COMMITTED transaction. `join()` short-circuits when `trx` is set, so the next
 * caller issued its queries against a dead handle. The reset now happens in a
 * `finally`, which is what keeps a THROWN commit from reinstating the bug.
 *
...
describe('build__UnitOfWork (media-core)', () => {
  ...
  describe('When a request abandons an open transaction', () => {
    it('should clear it on settle so the next request starts clean', async () => {
      const { uow, opened } = buildUow();
