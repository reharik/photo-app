...
describe('MediaItem (domain)', () => {
  ...
  // `applyProcessingResults` is no longer media-core's — it moved to
  // `@packages/worker-core` along with media asset ownership. Its three cases
  // (ready transition, not-processing rejection, replay rejection) now live in
  // `packages/context/worker-core/src/tests/mediaItem.domain.tests.ts`, run
  // against the aggregate that actually implements them.
});
