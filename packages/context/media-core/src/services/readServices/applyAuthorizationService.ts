...
import { ViewerMediaItemAuthzService } from './viewerReadServices/viewerMediaItemAuthzService';
import {
  DecoratedItem,
...
} from './viewerReadServices/viewerReadService.types';

type ApplyAuthorizationServiceDeps = {
  viewerMediaItemAuthzService: ViewerMediaItemAuthzService;
};
export type ApplyAuthorizationService = {
...
export const applyAuthorizationService = ({
  viewerMediaItemAuthzService,
}: ApplyAuthorizationServiceDeps): ApplyAuthorizationService => {
  // Albums
  ...
  const toItems = async <T extends HasId>(
    ...
    return viewerMediaItemAuthzService.addAuthzToItems(items);
  };
