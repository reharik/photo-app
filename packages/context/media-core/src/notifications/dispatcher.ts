...
export const build__NotificationDispatcher = ({
  ...
  return {
    ...
    processor: async (event) => {
      ...
      if (!strategy) {
        scopedLogger.warn('[NotificationDispatcher] no strategy for event kind — dropping event', {
      ...
      scopedLogger.info('[NotificationDispatcher] dispatching event', {
        eventKind: event.kind,
        actorId: event.actorId,
        occurredAt: event.occurredAt,
      ...
      if (!recipients.length) {
        scopedLogger.warn('[NotificationDispatcher] no recipients after guards — dropping event', {
          eventKind: event.kind,
      ...
      scopedLogger.info('[NotificationDispatcher] notification written', {
        eventKind: event.kind,
        kind: resolved.kind.value,
