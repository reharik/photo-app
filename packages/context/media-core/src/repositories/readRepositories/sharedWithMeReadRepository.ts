...
export type SharedWithMeReadRepository = {
  getSharedWithMe: (viewerId: string) => Promise<{
    sharedMediaItems: SharedMediaItemRow[];
    sharedAlbums: SharedAlbumRow[];
  }>;
};
...
export const buildSharedWithMeReadRepository = ({
  ...
  getSharedWithMe: async (viewerId: string) => {
    ...
    return { sharedMediaItems: mediaRows, sharedAlbums: albumRows };
  },
});
