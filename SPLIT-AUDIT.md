...
1. **The entire `worker-core/src/tests/` directory was dead.** Not one file in it tested
   `worker-core`. Two suites failed to run; the other four *passed while testing
   `media-core`* — they import `@packages/media-core` directly. I deleted 7 of the 8 files.
...
3. **Test files are not type-checked anywhere in this repo.** Both `tsconfig.json`s exclude
   `src/tests/**`, and `ts-jest` runs transpile-only. This is *the* reason the split's damage
...
Your instructions said both *"revert a pass if tests break"* and *"when uncertain, report
instead of acting."* **These conflict here, so I am saying so plainly rather than working
...
- The baseline was already red (2 suites failing to run). This is not a regression from
  green; it is a change in *failure mode*, from "broken tests" to "no tests".
...
dependency (§5.6), so this edge is invisible to the Nx graph too. *Acted on (2b).*
...
its own type imports pointed at nothing. *Acted on (2b).*
...
nothing flagged it because tests aren't type-checked. *Acted on (2c).*
...
`apps/api/src/tests/revokeAndReshare.integration.tests.ts` (§2.2) asserts that *a revoked
pending grant is invisible to `getPendingUserAuthorizationById`, so the sweep can never put a
dead invite token in an email.* That method was pruned from `media-core` — correctly, since no
...
- **The rename pass was thorough in production code but never ran over tests.** `worker-core`'s
  ...
  `systemAsyncNotificationRepository.ts` comment *was* correctly reworded to say "worker-core
