 ...
 *
 * DUPLICATED from apps/api/src/tests/ensureTestViewerUsers.ts on purpose (no shared
 * test-support package). Keep the row set identical in both projects.
 */
...
/**
 * Ensures rows exist in `user` for integration tests that persist rows referencing user ids.
  ...
  for (const row of TEST_USER_ROWS) {
    await database('user')
      .insert({
        ...
        // user_status became NOT NULL in migration 0019; seed users are active.
        userStatus: 'ACTIVE',
        emailVerified: true,
        createdBy: row.id,
        updatedBy: row.id,
