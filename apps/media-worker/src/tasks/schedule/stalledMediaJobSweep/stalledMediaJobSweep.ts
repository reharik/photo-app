...
export const build__StalledMediaJobSweep = ({
  ...
  return async () => {
    ...
    const { released, failed } = await uow.inTransaction(() =>
      mediaProcessingJobRepository.releaseStalledJobs(stalledBefore),
    );
    ...
    if (released > 0) {
      logger.warn(`[stalled-media-job-sweep] released ${released} stalled job(s) back to PENDING`);
    }
    if (failed > 0) {
      logger.error(
