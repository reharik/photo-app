...
import { ApplyEmailDeliveryEvents } from './applyEmailDeliveryEvents';
...
export interface ApplyEmailDeliveryEventsTask extends QueueWorkerTask {
  name: 'apply-email-delivery-events';
}

type ApplyEmailDeliveryEventsTaskDeps = {
  applyEmailDeliveryEvents: ApplyEmailDeliveryEvents;
};

export const build__ApplyEmailDeliveryEventsTask = ({
  applyEmailDeliveryEvents,
}: ApplyEmailDeliveryEventsTaskDeps): ApplyEmailDeliveryEventsTask => ({
  name: 'apply-email-delivery-events',
  type: 'queue',
  run: applyEmailDeliveryEvents,
  order: 10000,
});
